...
export async function ensureHostOrganization(
  ...
  const existingOrg = await db
    .prepare(
      'SELECT id FROM host_organizations WHERE owner_user_id = ? LIMIT 1',
    )
    .bind(userId)
    .first<{ id: string }>();
...
export async function createContest(
  ...
  const existing = await db
    .prepare(
      'SELECT id FROM contests WHERE host_organization_id = ? AND slug = ? LIMIT 1',
    )
    .bind(orgId, slug)
    .first();
 ...
 */
export async function getHostContests(
  userId: string,
): Promise<HostContestStats[]> {
  const db = await getDatabase();
